...
const isExistTodayDetail = computed(() => {
  return todayDetail.value ? true : false;
});
...
<template>
  <header id="app-header">
    ...
    <span class="tadayinfo" v-if="isExistTodayDetail">{{ today.year }}/{{ today.month }}/{{ today.day }}({{ todayDetail.week }})</span>
    <span class="tadayinfo" v-else>{{ today.year }}/{{ today.month }}/{{ today.day }}</span>
    <!-- https://koyomi.zingsystem.com/api/?mode=d&cnt=1&targetyyyy=2026&targetmm=3&targetdd=7 -->
